...
double currTiemParticulasFuente, lastTimeParticulasFuente;
	...
	while (psi) {
		...
		for(std::map<float, std::pair<std::string, glm::vec3> >::reverse_iterator it = blendingSorted.rbegin(); it != blendingSorted.rend(); it++){
			if(it->second.first.compare("aircraft") == 0){
				...
				currTiemParticulasFuente = TimeManager::Instance().GetTime();
				if (currTiemParticulasFuente - lastTimeParticulasFuente > 10.0) 
					lastTimeParticulasFuente = currTiemParticulasFuente;
				glDepthMask(GL_FALSE);
				glPointSize(10.0f);
				glActiveTexture(GL_TEXTURE0);
				glBindTexture(GL_TEXTURE_2D, textureParticleFountainID);
				shaderParticulasFountain.turnOn();
				shaderParticulasFountain.setFloat("Time", float(currTiemParticulasFuente - lastTimeParticulasFuente));
				shaderParticulasFountain.setFloat("ParticleLifetime", 18.0f);
				shaderParticulasFountain.setVectorFloat3("Gravity", glm::value_ptr(glm::vec3(0.0f, 0.9f, 0.0f)));
...
int main(int argc, char **argv) {
	init(1400, 820, "Window GLFW", false);
	applicationLoop();
	destroy();
	return 1;
}

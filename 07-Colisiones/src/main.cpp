...
bool processInput(bool continueApplication) {
	...
	if (glfwJoystickPresent(GLFW_JOYSTICK_1) == GLFW_TRUE) {
		...
		if (fabs(axes[1]) > 0.2f) {
			modelMatrixMayow = glm::translate(modelMatrixMayow, glm::vec3(0, 0, axes[1] * 0.5f));
			animationIndex = 0;
		}
		...
		if (fabs(axes[3]) > 0.2f) {
			camera->mouseMoveCamera(0.0, axes[3] * 0.5f, deltaTime);
		}
...
void applicationLoop() {
	...
	while (psi) {
		...
		if(currTime - lastTime < 0.016666667){//0.016666667
			glfwPollEvents();
			continue;
		}
		lastTime = currTime;
		...
		for (it = collisionDetection.begin(); it != collisionDetection.end(); it++) {
			...
			std::map < std::string, std::tuple<AbstractModel::OBB, glm::mat4, glm::mat4 >> ::iterator modeloBuscadoOBB = collidersOBB.find(it->first);
			std::map < std::string, std::tuple<AbstractModel::SBB, glm::mat4, glm::mat4 >> ::iterator modeloBuscadoSBB = collidersSBB.find(it->first);
...
int main(int argc, char **argv) {
	init(1200, 800, "Window GLFW", false);
	applicationLoop();
	destroy();
	return 1;
}
